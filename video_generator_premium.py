...
class PremiumVideoGenerator:
    ...
    def create_video(self, content: Dict[str, Any], topic: str, subtopic: str, filename: str = None) -> str:
        ...
        slides.append(self._gen_slide(temp_dir, "slide1.png", [
            {"text": "🩺 DAILY MEDICAL CASE", "size": 50, "color": self.colors['text'], "y": 500},
            {"text": "Can you diagnose this?", "size": 70, "color": self.colors['accent_q'], "y": 850, "bold": True},
            {"text": "Think before the answer", "size": 40, "color": self.colors['text'], "y": 1050}
        ...
        slides.append(self._gen_slide(temp_dir, "slide2.png", [
            {"text": "📌 Clinical Case", "size": 60, "color": self.colors['accent_case'], "y": 400},
            {"text": case_display, "size": 45, "color": self.colors['text'], "y": 750, "wrap": True}
        ...
        mcq_data = [{"text": "❓ Most Likely Diagnosis?", "size": 60, "color": self.colors['accent_q'], "y": 400}]
        for i, line in enumerate(mcq_lines):
            mcq_data.append({"text": line, "size": 42, "color": self.colors['text'], "y": 700 + (i * 130), "wrap": True})
        ...
        slides.append(self._gen_slide(temp_dir, "slide4.png", [
            {"text": "⏳ Think for 5 seconds", "size": 65, "color": self.colors['text'], "y": 950}
        ...
        slides.append(self._gen_slide(temp_dir, "slide5.png", [
            {"text": f"✅ Correct Answer: {ans_key}", "size": 75, "color": self.colors['accent_a'], "y": 450},
            {"text": ans_text, "size": 50, "color": self.colors['text'], "y": 600, "wrap": True},
            {"text": "🧠 Mnemonic", "size": 55, "color": self.colors['accent_case'], "y": 950},
            {"text": content['mnemonic'], "size": 42, "color": self.colors['text'], "y": 1100, "wrap": True}
        ...
        slides.append(self._gen_slide(temp_dir, "slide6.png", [
            {"text": "Daily medical cases", "size": 60, "color": self.colors['text'], "y": 850},
            {"text": "Follow for more", "size": 70, "color": self.colors['accent_q'], "y": 1050, "bold": True}
    ...
    def _gen_slide(self, temp_dir, name, elements, duration, audio_text):
        ...
        for el in elements:
            ...
            if el.get('wrap'):
                lines = self._wrap_text(text, f, self.width - 200)
                current_y = y
                for line in lines:
